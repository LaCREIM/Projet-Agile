...
const EvaluationHome = () => {
  ...
  const [filteredEvaluations, setFilteredEvaluations] = useState<GetEvaluationDTO[]>([]);
                    ...
                    (evaluation: GetEvaluationDTO, index: number) => (
                        ...
                        >
                          ...
                          <td className="flex gap-3 justify-center items-center">
                            <div className={"tooltip"} data-tip="Dupliquer l'évaluation">
                              <FontAwesomeIcon
                                  icon={faCopy}
                                  className={`text-black text-base cursor-pointer ${evaluation?.droit?.duplication == "O" ? '' : 'text-gray-400 cursor-not-allowed'} `}
                                  onClick={() => evaluation?.droit?.duplication == "O" && handleDuplicate(evaluation.evaluation.idEvaluation)}
                              />
                            </div>
                            <FontAwesomeIcon
                                icon={faEye}
                          ...
                          >
                            <DuplicateEvaluationConfirmation
                                evaluation={evaluation.evaluation}
                                onClose={() => closeModal(`duplicate-${evaluation.evaluation.idEvaluation}`)}
                                onConfirm={() => confirmDuplicate(evaluation.evaluation.idEvaluation)}
                            />
                          </dialog>
                        </tr>
                    )
