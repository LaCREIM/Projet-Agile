...
const AddEvaluation = ({ promotions, onClose }: AddEvaluationProps) => {
  ...
  const initEvaluation = {
    ...
    prenomEnseignant: "",
    idEvaluation: 0,
    noEnseignant: localStorage.getItem("id")
    ...
    if (name === "noEvaluation") {
      if (parseInt(value) < 1 || parseInt(value) > 99) {
        setErrors((prevErrors) => ({
          ...prevErrors,
          noEvaluationError:
            "Le numéro de l'évaluation doit être compris entre 1 et 99.",
        }));
      } else {
        setErrors((prevErrors) => ({
          ...prevErrors,
