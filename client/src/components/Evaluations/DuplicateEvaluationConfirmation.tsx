...
const DuplicateEvaluationConfirmation: React.FC<
  DuplicateEvaluationConfirmationProps
> = ({ evaluation, promotions, onClose, onConfirm }) => {
  ...
  const handleChange = (
    ...
    if (name === "debutReponse" || name === "finReponse") {
      const debut =
        name === "debutReponse" ? value : updatedEvaluation.debutReponse;
      const fin = name === "finReponse" ? value : updatedEvaluation.finReponse;
        ...
        <div className="space-y-4">
          ...
          <label className="flex flex-row items-center col-span-2 gap-2">
            <select
              ...
              name="promotion"
              value={
                updatedEvaluation.anneeUniversitaire &&
                updatedEvaluation.codeFormation
          ...
          <div className="flex flex-col gap-2">
            ...
            {errors.debutReponseError && (
              <p className="text-red-500 text-sm">{errors.debutReponseError}</p>
            )}
          </div>
          <div className="flex flex-col gap-2">
            <label className="input input-bordered flex items-center gap-2 w-full">
            ...
            {errors.finReponseError && (
              <p className="text-red-500 text-sm">{errors.finReponseError}</p>
            )}
          </div>
        </div>
        <h1 className="text-sm text-red-500 mt-2">
          * Veuillez bien vérifier les informations ci-dessus!
        </h1>
        <div className="modal-action">
          ...
          <button
            ...
            disabled={
              !!errors.debutReponseError || 
              !!errors.finReponseError || 
              !updatedEvaluation.noEvaluation ||
              !updatedEvaluation.designation || 
              !updatedEvaluation.anneeUniversitaire || 
              !updatedEvaluation.codeFormation || 
              !updatedEvaluation.codeUE     
            }
          >
            Dupliquer
          </button>
