...
import {
  fetchHashedReponseEvaluationAsyncETD,
  fetchReponseEvaluationAsyncETD,
  getReponseEvaluationETD,
    ...
    <div className="w-full min-h-screen p-5 bg-gray-100 overflow-y-auto">
      ...
      <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold mb-4">
          {evaluation?.designation || "Aucune désignation"}
        </h2>
        <div className="grid grid-cols-2 gap-4 text-gray-600">
          <div className="space-y-3">
            <div>
              <strong>Enseignant :</strong> {evaluation?.nomEnseignant}{" "}
              {evaluation?.prenomEnseignant}
            </div>
            <div>
              <strong>Période :</strong> {evaluation?.periode}
            </div>
            <div>
              <strong>Début des réponses :</strong> {evaluation?.debutReponse}
            </div>
            <div>
              <strong>Fin des réponses :</strong> {evaluation?.finReponse}
            </div>
          </div>
          <div className="space-y-3">
            <div>
            ...
            <div>
              <strong>Promotion :</strong> {evaluation?.nomFormation} -{" "}
              ...
              <ul className="space-y-3 pl-0">
                {rubrique.questions.map((question) => (
                  <li
                    ...
                    className="grid grid-cols-4 gap-1 items-center border-gray-200 py-2"
                  >
